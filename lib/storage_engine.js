...
fnHistory = function(ns, pk, queryStmt, option) {
  ...
  return CmsOplog.find(query, opt);
};
 ...
 * @return {object}      快照数据
 */
fnSnapshot = function(ns, pk, snapshotId, option) {
  // master version
  var dbInfo = extractDbInfo(ns);
  var conn = getMongoClient(dbInfo.dbName, dbInfo.collName);
  var masterData;
  if (option === undefined)
