...
fnUpdate = function(ns, pk, o, op, custom) {
  ...
  Meteor.call('TaskPool.update.status', entry.pk);
...
var scrollBack = function(current, oplog) {
  var temp = current,
      origin = oplog.original;
  temp = _.extend(temp, _.omit(origin, '_id'));
  return temp;
};
 ...
 * @param  {object} snapshotId  快照的版本。如果为undefined，表示取最新的版本。否则为回滚到指定版本。
 * @return {object}      快照数据
 */
fnSnapshot = function(ns, pk, option, snapshotId) {
  ...
  // TODO: snapshotId 存在，则回滚，不存在则是应用当前的状态为staged数据
  var query, opt, actFunction;
  if (snapshotId !== undefined) {
    var list = OplogPkList.findOne({'pk': pk._str});
    if(list.branch && list.branch.length === 1){
      query = {'snapshotId': 1, 'status': 'merged'};    //  query by snapshot
    }else{
      query = {'snapshotId': {'$gt': snapshotId}, 'status': 'merged'};    //  query by snapshot
    }

    opt = {
      fields: {'o': 1, 'original': 1},  // use original
      sort: {'ts': -1}                  // use desc
    };
    actFunction = scrollBack;           // use function: scrollBack
  }else{
    query = {'status': 'staged'};       // query newest edit info
    opt = {
      fields: {'o': 1, 'op': 1},        // use op
      sort: {'ts': 1}                   // use asc
    };
    actFunction = apply;                // use function: apply
  }
  ...
  for (var idx in history) {
    ...
    revData = actFunction(revData, oplog);
  }
