...
Router.route('/review-task/:type/:taskId', {
  name: 'taskRecieve',
  // template: 'review' + this.params.type,
  title: '接收任务',
  parent: 'home',
  controller: 'TaskRecieveController',
  // waitOn: function() {
  //   return [Meteor.subscribe('editor'), Meteor.subscribe('recieveTask', this.params.taskId)];
  // },
  // data: function() {
  //   return;
  // }
...
TaskRecieveController = RouteController.extend({
  template: 'reviewViewSpot' + this.params.type,
  waitOn: function () {
    return [Meteor.subscribe('editor'), Meteor.subscribe('recieveTask', this.params.taskId)];
  },
  data: function () {
