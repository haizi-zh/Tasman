...
reviewViewspotBaseController = RouteController.extend({
  waitOn: function() {
    ...
    return Meteor.subscribe('viewspot', this.isAbroad(), this.params.zoneName, this.pageLimit());
  },
  increment: 10,
  isAbroad: function() {
    return this.params.countryRegion !== "china";
  ...
  subscriptions: function() {
    this.viewspotsSub = Meteor.subscribe('viewspot', this.isAbroad(), this.params.zoneName, this.pageLimit());
  },
  viewspots: function() {
...
Router.route('/ViewSpot/:id', {
  name: 'viewspotDetail',
  waitOn: function() {
    return Meteor.subscribe('viewspotDetail', this.params.id);
  },
  data: function() {
    Session.set('oplog', {});
    var Items = [];
...
Router.route('/Locality/:id', {
  name: 'localityDetail',
  waitOn: function() {
    return Meteor.subscribe('localityDetail', this.params.id);
  },
  data: function() {
    Session.set('oplog', {});
    var Items = [];
...
/****************** review-hotel-begin *****************/
reviewHotelBaseController = RouteController.extend({
  increment: 10,
  waitOn: function() {
    // if (!this.params.zoneName) {
    //   this.render('reviewViewspot');
    // }
    // TODO 一开始返回城市列表和所有工作的进度
    return Meteor.subscribe('hotels', this.isAbroad(), this.params.zoneName, this.pageLimit());
  },
  isAbroad: function() {
    return this.params.countryRegion !== "china";
  },
  ...
  subscriptions: function() {
    this.hotelsSub = Meteor.subscribe('hotels', this.isAbroad(), this.params.zoneName, this.pageLimit());
  },
  hotels : function() {
    return Hotel.find(this.findOptions().query, this.findOptions().limit);
  },
  data: function() {
    var hasMore = this.hotels().count() === this.pageLimit();
    return {
      isAbroad: this.isAbroad(),
      hotels: this.hotels(),
      ready: this.hotelsSub.ready,
      nextPath: hasMore ? this.nextPath() : null
    };
  }
});

reviewHotelController = reviewHotelBaseController.extend({});

Router.route('/data-review/hotel/:countryRegion/:zoneName?/:pageLimit?', {
  name: 'reviewHotel',
  controller: reviewHotelController,
  title: '酒店审核',
  parent: 'dataReview'
})
/****************** review-hotel-end *****************/
...
Router.route('/recheck', {
  ...
  parent: 'home',
  waitOn: function(){
    return [Meteor.subscribe('editor'), Meteor.subscribe('oplog-pk-list')]
  },
  data: function(){
