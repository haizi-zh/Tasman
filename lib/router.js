...
Router.route('/data-review/plan', {
  ...
  parent: 'dataReview',
  waitOn: function(){
    return Meteor.subscribe('planDetail', '54dc629656eb6f2af42dd609');
  },
});
...
Router.route('/Plan/:id', {
  ...
  parent: 'home',
  waitOn: function(){
    return [Meteor.subscribe('planDetail', this.params.id), Meteor.subscribe('oplog', 'plan.Plan', new Mongo.ObjectID(this.params.id), 0)];
  },
  data: function(){
    Session.set('oplog', {});
    var Items = [];
    var detailInfo = storageEngine.snapshot('plan.Plan', new Mongo.ObjectID(this.params.id));
    // if(detailInfo){
    //   review('Plan', detailInfo, Items);
    //   createOriginTextMD5(Items);
    // }
    // 激活获取图片
    Meteor.subscribe("Images", this.params.id);
    Session.set('currentPlanId', this.params.id);
    return {
      'planDetail': Items
    };
  }
